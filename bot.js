...
client.on("userUpdate", async (oldUser, newUser) => {
  ...
  const tag = 'TAG' //Tag
  const sunucu = 'SUNUCU ID' //Sunucu
  const kanal = 'KANAL ID' //Kanal
  const rol = 'ROL ID' //Rol
  try { 
...
moment.locale("tr");
let kanal = client.channels.cache.get(`725786943987908609`) //Kayıt Kanalı ID
await kanal.send(`>>>  ${member} Sunucumuza Hoş Geldin! \n\n Seninle Birlikte ${member.guild.memberCount} Kişiyiz\n\n Birazdan <@&716922783157977178> Rolündeki Yetkililer Sizi Kayıt Edecek Lütfen Bekleyin\n\n Hesabın Oluşturulma Tarihi: ${moment(member.user.createdAt).format("DD MMMM YYYY, dddd (hh:mm)")}\n\n **__${member.guild.name}__** `, new Discord.MessageAttachment("https://media.discordapp.net/attachments/716981345854816267/720983955092013076/Talha_Gif.gif"," kayit.gif")).catch(e => console.log(e))
}); 
...
let botrol = '728250056032911420' //Bot otorol          
let üyerol = '718845493891039242'//Kullanıcı otorol
  if (member.user.bot) {
